...
public class ScheduleJobServiceImpl extends ServiceImpl<ScheduleJobDao, ScheduleJobEntity> implements ScheduleJobService {
    ...
    public int updateBatch(Long[] jobIds, int status) {
        ScheduleJobEntity scheduleJobEntity1;
        Integer result = null;
        for (Long jobId : jobIds) {
            QueryWrapper<ScheduleJobEntity> queryWrapper = new QueryWrapper<>();
            queryWrapper.eq("job_id", jobId);
            ScheduleJobEntity scheduleJobEntity = baseMapper.selectOne(queryWrapper);
			...
			result = baseMapper.updateById(scheduleJobEntity1);
        }
        return result;
    }
